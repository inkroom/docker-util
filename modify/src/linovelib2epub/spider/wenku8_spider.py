...
class Wenku8Spider(BaseNovelWebsiteSpider):
    ...
    def fetch(self,novel: LightNovel) -> LightNovel:
        novel = asyncio.run(self._fetch(novel))
        return novel

    async def _fetch(self,novel: LightNovel) -> LightNovel:
        ...
        async with aiohttp.ClientSession(trust_env=trust_env, timeout=timeout, connector=conn,
            ...
            if len(novel.book_title) == 0:
                novel, catalog_url = await self._fetch_basic_info(session,novel)
                self._catalog_url = catalog_url
            else:
                self._catalog_url = f'{self.spider_settings["base_url"]}/novel/1/{self.spider_settings["book_id"]}/index.html'

            await self._fetch_catalog_content(session, novel)
            return novel

    async def _fetch_basic_info(self, session,novel: LightNovel) -> tuple[LightNovel, str]:
        # index url
