    ...
    if 下载图片 and 分卷输出:
        ...
        for 卷名 in 内容:
            ...
            if not 文件存在:
                ...
                os.makedirs("file")
            下载图片集合(图片URL集合[卷名], 4)
            写到书本(书名+" "+卷名, 作者, 内容[卷名], "-".join(clean_file_name(封面URL).split("/")[-4:]), "file/" + "-".join(clean_file_name(封面URL).split("/")[-4:]), "file", 'data/'+书名, True)
            try:
                shutil.rmtree('file')
            except:
                pass
        ...
        if not 文件存在:
            ...
            os.makedirs("file")
        下载文件(封面URL)
        写到书本(书名, 作者, 内容, "-".join(clean_file_name(封面URL).split("/")[-4:]), "file/" + "-".join(clean_file_name(封面URL).split("/")[-4:]), "file","data")
    try:
        shutil.rmtree('file')
    except:
        pass
