...
WORKDIR /app
RUN mkdir /plib && cd /plib && sed -i 's/dl-cdn.alpinelinux.org/mirrors.ustc.edu.cn/g' /etc/apk/repositories && apk add gcc g++ git jpeg jpeg-dev zlib zlib-dev vim curl && git clone https://github.com/lightnovel-center/linovelib2epub.git && cd linovelib2epub && git checkout d5885af
COPY modify/ /plib/linovelib2epub/
RUN cd /plib  && cd linovelib2epub && git checkout -b test && git add . && git config user.name "test" && git config user.email "test@test.com" && git commit -m "test"
